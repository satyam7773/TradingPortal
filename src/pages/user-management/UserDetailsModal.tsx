...
const UserDetailsModal: React.FC<UserDetailsModalProps> = ({ user, onClose, onToggle, depth = 0 }) => {
  ...
  console.log("Modal mounted with user prop:", user);
  ...
  //       console.log('✅ User details loaded successfully',response.data);
  //     } else {
  ...
  const fetchUserDetails = useCallback(async (targetUser?: UserData) => {
    // Fix 1: If neither exists, OR if the ID is 0/null/undefined, STOP.
    const u = targetUser || user;
    if (!u || !u.id || u.id === '0') return;
    ...
    try {
      ...
      // Fix 2: Double check parseInt result
      const numericId = parseInt(u.id);
      if (isNaN(numericId)) return;

      const response = await userManagementService.fetchUserDetails(numericId);

      if (response?.responseCode === '0' || response?.responseCode === '1000') {
        setUserDetails(response.data);
      } else {
        setError(response?.responseMessage || 'Failed to fetch user details');
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    // Fix 3: Only trigger if we actually have a valid ID
  ...
  console.log("DEBUG: Modal User Data ->", selectedUserForAddCredits);
              ...
              <div>
                ...
                <div className="flex justify-around items-center">
                  <div className="text-center">
                    ...
                    <p className="text-base font-bold text-gray-900 dark:text-white">{(selectedUserForAddCredits?.credits || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs font-semibold mb-1 text-gray-500 dark:text-gray-400">New</p>
                    <p className={`text-base font-bold ${(
                      creditTransType === 'Credit'
                        ? (selectedUserForAddCredits?.credits || 0) + (Number(creditAmount) || 0)
                        : (selectedUserForAddCredits?.credits || 0) - (Number(creditAmount) || 0)
              ...
              onClick={async () => {
                ...
                try {
                  const payload = {
                    ...
                    userId: Number(selectedUserForAddCredits.userId),
                    comments: creditNote || '',
                  ...
                  console.log('selectedUserForAddCredits', selectedUserForAddCredits)
                  if (code === '0' || code === '1000') {
