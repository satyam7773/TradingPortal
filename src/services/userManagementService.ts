...
class UserManagementService {
   ...
   */
  async fetchTradeMarginSettings(
    userId: number,
    exchange: string,
  ): Promise<any> {
   ...
   * Fetch exchanges
   */
  async fetchExchanges(userId?: number | string): Promise<
    Array<{
      name: string;
      turnover: boolean;
      lot: boolean;
      groupId: number;
      intraDaySquareOff?: boolean;
    }>
  > {
    ...
    }>(`${this.baseUrl}/portal/exchanges/v2`, request);
  ...
  async updateIntraDaySquareOff(
    ...
    const response = await apiClient.post<{
      ...
      responseMessage: string;
      data: any;
    }>(`${this.baseUrl}/settings/updateIntraDaySquareOff`, request);
   ...
   * Fetch symbols for trading
   */
  async fetchAllSymbols(exchange?: string, userId?: number): Promise<any> {
    // const userDataStr = localStorage.getItem('userData')
    // const storedUserData = userDataStr ? JSON.parse(userDataStr) : null
    // const userId = storedUserData?.userId || 2
   ...
   */
  async fetchTradeSymbolsReport(
    userFilterType: "ALL" | "SINGLE" = "ALL",
    exchange?: string,
  ): Promise<any> {
    try {
   ...
   */
  async fetchUserPositionsForExchange(
    exchange: string,
    token: number,
    userId: number,
  ): Promise<any> {
   ...
   * Fetch own users (for user dropdown)
   */
  async fetchOwnUsersforOrders(userId: number): Promise<any> {
    const request = {
      userId,
    };
  ...
  async fetchM2MReport(
    userId: number | string,
    userFilterType: "ALL" | "SINGLE",
  ): Promise<any> {
    try {
  ...
  async updateCarryForwardMargin(
    userId: number,
    exchanges: any[],
  ): Promise<any> {
    const payload = {
